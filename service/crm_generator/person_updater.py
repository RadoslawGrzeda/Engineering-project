...
from service.crm_generator.person_generator import (
    SCHEMA_VERSION,
    SOURCE_SYSTEM,
    LOYALTY_STATUSES,
    CIVIL_STATUSES,
...
class PersonUpdater:
    ...
    def _build_payload(self, customer, loyalty, languages, nationalities, indicator,
        ...
        subscriptions = []
        for s in subs:
            unsub = s.get("date_of_unsubscription")
            subscriptions.append({
                "person_id": person_id,
                "communication_code": s.get("communication_code"),
                "value": "AIV_02" if unsub else "AIV_01",
                "date_of_subscription": _fmt_dt(s.get("date_of_subscription")),
                "date_of_unsubscription": _fmt_dt(unsub),
