...
logger = get_logger('service.streamlit.streamlit_app.py', service="streamlit")
...
class Streamlit:
    ...
    def _show_user_history(self):
        query = text("""
            ...
            FROM (
                SELECT * FROM meta.product_etl_load_log
                UNION ALL
                SELECT * FROM meta.store_etl_load_log
            ) AS etl_load_log
    ...
    def _send_file_information_to_db(self, user_name:str, destination_table:str,
        ...
        sql = text(f'''
                INSERT INTO meta.{db_schema}_etl_load_log(
