...
public class MetadataSink extends JdbcProcessSink<Transaction> {

    public static final OutputTag<DeadLetter> DEAD_LETTER =
            new OutputTag<>("metadata_dead_letter", TypeInformation.of(DeadLetter.class));

    public static final String SQL =
            "INSERT INTO transaction.transaction_metadata " +
            "(metadata_id, transaction_id, comments, import_field, shared, user_loan, correlation_id) " +
            "VALUES (?, ?, ?, ?, ?, ?, ?) " +
    ...
    protected JdbcStatementBuilder<Transaction> getStatementBuilder() {
        return (PreparedStatement ps, Transaction tx) -> {
            ...
            Transaction.Metadata m = tx.getMetadata();
            ps.setString(1, m.getMetadataId());
            ps.setString(2, tx.getTransactionId());
            ps.setString(3, m.getComments());
            ps.setString(4, m.getImportField());
            if (m.getShared() != null) {
                ps.setBoolean(5, m.getShared());
            } else {
                ps.setNull(5, java.sql.Types.BOOLEAN);
            }
            if (m.getUserLoan() != null) {
                ps.setBoolean(6, m.getUserLoan());
            } else {
