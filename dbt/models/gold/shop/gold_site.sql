...
site_contact as (
    select
        ...
        groupArray(valid_to)   as contact_valid_to
...
select
    ...
    1 as is_current,
    now() as created_at,
    greatest(site.updated_at, info.updated_at) as updated_at
from site_main site
left join site_info info on site.site_unique_code = info.site_unique_code
left join site_format format on site.site_unique_code = format.site_unique_code
