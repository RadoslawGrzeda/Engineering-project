...
def _dbt_op(task_id: str, select: str) -> DockerOperator:
    ...
    return DockerOperator(
        task_id=task_id,
        image=DBT_IMAGE,
        command=f"run --select {select}{full_refresh_flag} --profiles-dir /dbt",
        ...
        environment={
            "DBT_CLICKHOUSE_HOST": os.getenv("CLICKHOUSE_HOST"),
            "DBT_CLICKHOUSE_PORT": os.getenv("CLICKHOUSE_HTTP_PORT"),
...
@dag(
    ...
    max_active_runs=1,
    params={
        "correlation_id": Param("", type="string", description="Correlation ID passed from trigger"),
    },
    tags=["silver", "gold", "dbt", "product"],
)
def dbt_transform_product():
    silver = _dbt_op("silver", "path:models/silver/product")
    gold = _dbt_op("gold", "path:models/gold/product")
    silver >> gold


dag = dbt_transform_product()
